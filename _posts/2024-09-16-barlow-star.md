...
# Barlow 인증 모듈 설계와 AI 법안 요약 파이프라인 안정화

---

## Situation (상황)

바로(Barlow)는 "오늘 나온 법안"을 사용자에게 쉽게 제공하는 서비스입니다. 서비스 초기 단계에서 두 가지 과제가 동시에 놓여 있었습니다.

하나는 인증 구조였습니다. JWT 기반 인증뿐 아니라 게스트 로그인, 추후의 OAuth·외부 인증까지 열려 있어야 했고, 인증 기능 자체가 나중에 API Gateway나 별도 모듈로 분리될 가능성도 고려해야 했습니다.

다른 하나는 생성형 AI를 이용한 법안 요약 기능이었습니다. 2인 개발 체제이고 아직 뚜렷한 비즈니스 모델이 없는 상황이라, 유지비와 개발 시간을 최소화하면서도 LLM 호출의 불안정성(응답 실패, 지연)에 흔들리지 않는 구조가 필요했습니다.

---

## Task (과제)

1. 다양한 인증 방식(JWT, 게스트, 추후 OAuth)에 열려 있으면서도 별도 모듈로 분리 가능한 인증 구조를 설계할 것
2. AI 법안 요약 기능을 기존 배치 파이프라인에 통합하되, 유지비와 개발 시간을 최소화할 것
3. 생성형 AI 응답의 예측 불가능한 지연(Outlier)에도 전체 배치가 흔들리지 않는 완료 감지·수집 전략을 세울 것
4. 폴링 단계의 처리 방식과 스레드 수를 실측을 통해 최적화할 것

---

## Action (행동)

### 1) 인증 방식 확장을 위한 Credential/Authenticator 추상화

바로 서비스에서는 요청의 자격 증명을 **Authentication**으로 처리합니다. 자격 증명은 `Credential` 인터페이스의 구현체로 정의되며, `Authenticator`는 다양한 Credential 방식을 지원할 수 있도록 설계했습니다.
...
인증 모듈은 `Credential`/`Authenticator` 인터페이스 기반으로 추상화되어, JWT 외의 인증 방식이나 별도 모듈로의 분리에도 열려 있는 구조로 완성했습니다. RSA 기반 서명/검증 분리로 향후 MSA 환경에서 공개키만으로 검증 책임을 분리할 수 있는 여지도 확보했습니다.

AI 법안 요약 파이프라인은 생성 실패를 예외가 아닌 정상 분기로 다루도록 설계하고, FIFO BlockingQueue 기반 폴링과 4개 스레드 구성을 통해 outlier(최대 40초 이상 지연되는 요청)가 섞여 있어도 전체 배치가 약 17초 내외로 안정적으로 완료되는 구조를 갖췄습니다. 스레드를 8개로 늘려도 유의미한 개선이 없다는 것을 실측으로 확인해, 4스레드가 비용 대비 가장 합리적인 설정임을 검증했습니다.
