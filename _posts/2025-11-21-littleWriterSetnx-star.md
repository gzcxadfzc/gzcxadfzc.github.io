...
# Redis SETNX 기반 BookInProgress 동시성 제어
...
이를 방지하기 위해 **Redis SETNX 기반 락**을 사용하여 동시에 하나의 스레드만 특정 책에 대한 작업을 수행할 수 있도록 제어하기로 했습니다.
...
불변(Immutable) 타입으로 Java Record로 구현되어 모든 필드가 final이고, Status enum을 통해 상태 전이를 명확히 관리하며, `List<BookPage>`로 페이지 순서를 보장합니다.

---

## Task (과제)

1. 동일한 책(BookInProgress)에 대한 동시 요청 중 정확히 하나만 성공하도록 원자적으로 락을 제어할 것
2. 서버 다운이나 예외로 락 해제가 누락되더라도 데드락이 발생하지 않도록 TTL로 방지할 것
3. 다른 스레드가 획득한 락을 실수로 해제하지 않도록 안전장치를 둘 것
4. AI 페이지 생성과 책 완성 처리 두 흐름 모두에 동일한 락 메커니즘을 적용할 것

---

## Action (행동)

### 1) RedisLockManager — 핵심 락 관리자
...
#### 3.1) BookProgressService — AI 페이지 생성
...
#### 3.2) BookCompleteExecutor — 책 완성 처리
...
기본적인 락 획득과 자동 해제를 검증합니다.

**다중 스레드 — 동시에 같은 리소스에 대한 락 획득 시도 시 하나만 성공**
...
10개 스레드 중 **1개만 성공**(SETNX 성공)했고, 나머지 **9개는 실패**(BookProgressException 발생)했습니다.
...
예외가 발생해도 `finally` 블록에서 락이 안전하게 해제되어, 이후 시도가 정상적으로 성공함을 확인했습니다.
