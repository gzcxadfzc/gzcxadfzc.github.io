...
# LittleWriter 아키텍처 리팩터링: 도메인 분리, 트랜잭션 경계 재설정, N+1 제거

---

## Situation (상황)

LittleWriter의 도메인 로직이 특정 AI 서비스(OpenAI)에 강하게 결합되어 있어, AI 모델을 바꾸려면 도메인 코드까지 함께 수정해야 했습니다. 핵심 비즈니스 규칙과 기술 구현의 경계가 모호해진 상태였습니다.

책 저장 흐름에서도 상위 서비스 계층에 선언된 `@Transactional` 스코프 안에 Redis 조회, MySQL 저장, S3 업로드까지 함께 묶여 있어 트랜잭션이 필요 이상으로 길어지고 있었습니다.

조회 성능 측면에서는 `BookJpaEntity`, `BookCharacterJpaEntity`, `BookPageJpaEntity`를 JPA 연관관계(`@OneToOne`, `@OneToMany`)로 직접 연결해두어, Book을 조회할 때마다 Character·Pages를 N번씩 추가 조회하는 N+1 문제가 발생하고 있었습니다.

## Task (과제)

1. 생성형 AI 기술의 빠른 변화(GPT-3 → GPT-4 → GPT-4o)와 비용 최적화를 위한 모델 교체에 대비해, 도메인 계층을 특정 AI 서비스 구현으로부터 분리할 것
2. 트랜잭션 롤백 불가·응답 지연·계층 노출 문제를 해결하기 위해 트랜잭션 경계를 인프라 레이어로 재설정할 것
3. ORM 연관관계로 인한 N+1 문제를 제거하고, 조회 방식을 쿼리 수준에서 직접 제어할 수 있는 구조로 바꿀 것

---

## Action (행동)

### 1) 도메인과 AI 모듈 분리 — 인터페이스 기반 아키텍처

`BookInProgress`(진행중인 책)과 `Book`(완성된 책)을 도메인 엔티티로 정의하고, 계층을 다음과 같이 분리했습니다.
...
Book, BookInProgress 등을 Aggregate로 활용하여 도메인 규칙을 설정했습니다.
...
### 2) 트랜잭션 경계 재설정 — IO를 트랜잭션 밖으로

#### 2.1) 문제 — 상위 레이어에 노출된 트랜잭션과 IO 작업
...
- **트랜잭션 실패 시 IO 롤백 불가**: DB 트랜잭션이 롤백되더라도 이미 업로드된 이미지나 외부 시스템 호출 결과는 되돌릴 수 없습니다. DB 상태와 외부 저장소 상태가 불일치하는 문제가 발생할 수 있습니다.
- **응답 속도 증가 및 병목 가능성**: 트랜잭션이 길어지면서 DB 커넥션이 필요 이상 오래 점유되어 커넥션 풀이 빠르게 소진(pool exhaustion)되어 동시 처리량이 감소합니다. 결과적으로 전체 요청의 응답 속도가 증가하고, 순간 부하 시 병목이 발생하여 단일 요청 지연뿐 아니라 서비스 전반의 안정성까지 저하됩니다.
- **트랜잭션 경계가 상위 계층에 노출**: RDBMS용 `@Transactional` 어노테이션이 서비스 계층에 선언되어 있어, 서비스 레이어가 저장 전략에 종속됩니다. 도메인 계층의 역할을 흐리게 하며, 유지보수성과 테스트 용이성도 떨어지는 문제가 있었습니다.
...
이를 통하여 기존 동화 저장 응답시간을 평균 600ms에서 140ms 정도로 단축할 수 있었습니다.

#### 2.3) 트랜잭션 경계 설정 — 인프라 레이어로 이동

기존 구조에서는 상위 서비스 계층에서 `@Transactional`을 선언하여 트랜잭션을 시작하였으나 이 방식은 여러 문제가 있습니다.
- 구체적인 인프라 정보가 상위 레이어에 노출됩니다.
...
`BookInProgress`로부터 어떤 방식으로 `Book` 애그리거트를 생성할지는 도메인 규칙이 완전히 책임지며, 인프라 계층(`BookRepositoryAdapter`)은 도메인이 만들어 준 `Book`을 트랜잭션 내부에서 MySQL 테이블에 저장하기만 합니다.

이를 통하여 **"Book을 어떻게 만들지는 도메인에서 결정하고, 만들어진 Book을 어떻게 저장할지는 인프라에서 처리하는 구조"**로 역할을 명확히 분리하였습니다.

### 3) 책 조회 성능 개선 — N+1 제거

#### 3.1) ORM 연관관계 기반 N+1 문제
...
- **N + 1 문제**: Book 조회 1번에 각 Book마다 Character, Pages 조회 N번이 추가로 발생합니다.
- **조회 형태가 고정되어 유연성 부족**: 도메인 요구사항이 바뀌면 ORM 매핑 전체를 수정해야 합니다.
- **애그리거트 변경 시 영향 범위가 커지는 구조**: Book 도메인 설계 변경이 JPA/Hibernate 설정까지 전파됩니다.

결과적으로 ORM 연관관계를 유지하는 구조는 도메인/쿼리 최적화 양쪽에 모두 불리하다고 판단했습니다. 따라서 JPA는 단순하게 `RDBMS 테이블 – Java 객체` 매핑 역할만 담당하도록 축소하고, 조회시 JPQL 기반으로 최적화된 형태로 가져오는 방식으로 변경하였습니다.

#### 3.2) JPQL을 사용한 DTO Projection

`BookJpaEntity`와 `BookCharacterJpaEntity`는 조회 시점에 JPQL을 사용해 조인하고, 결과를 `BookCharacterProjection`로 Projection 하여 한 번에 가져오도록 변경하였습니다.
...
#### 3.3) 도메인 애그리거트를 통한 반환

`BookPageJpaEntity`는 `BookJpaEntity`과의 `@OneToMany` 연관관계를 사용하지 않고, bookId를 기준으로 하는 명시적인 조회 메서드를 사용하여 한 번에 전체 페이지를 조회하도록 변경하였습니다.
...
- 동화 저장 응답시간을 평균 **600ms → 140ms**로 단축했습니다 (트랜잭션에서 S3 업로드를 분리하고 `AFTER_COMMIT` 비동기 처리로 전환한 결과).
- 도메인 계층이 AI 모듈 구현으로부터 인터페이스로 분리되어, AI 모델 교체 시 도메인 코드를 수정할 필요가 없어졌습니다.
- 트랜잭션 경계를 인프라 레이어로 이동시켜, 트랜잭션 실패 시 IO 롤백 불가 문제와 커넥션 풀 소진 위험을 구조적으로 제거했습니다.
- Book 조회 시 JPA 연관관계 대신 JPQL Projection + 명시적 페이지 조회로 전환하여, Lazy 로딩으로 인한 암묵적 N+1 문제를 근본적으로 제거했습니다.
